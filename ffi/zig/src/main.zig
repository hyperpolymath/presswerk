...
export fn presswerk_process(handle: ?*Handle, input: u32) Result {
...
export fn presswerk_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn presswerk_free_string(str: ?[*:0]const u8) void {
...
export fn presswerk_process_array(
    handle: ?*Handle,
    buffer: ?[*]const u8,
    len: u32,
) Result {
...
/// Register a callback
export fn presswerk_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
test "error handling" {
    const result = presswerk_process(null, 0);
    try std.testing.expectEqual(Result.null_pointer, result);

    const err = presswerk_last_error();
